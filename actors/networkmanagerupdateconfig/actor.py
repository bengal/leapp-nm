...
# On RHEL7 NetworkManager used by default the 'dhclient' DHCP backend,
# while it's the 'internal' one on RHEL8. We want to keep 'dhclient'
# enabled on upgrade, unless the user explicitly choose another
# backend in the configuration. To do so, we drop a configuration
# snippet in /usr/lib.
